...
// When the button is pressed it will change to a new pixel
...
#define BUTTON_PIN   5                        //Push button used to start animation
#define BRIGHTNESS_UP_BUTTON_PIN   16         //Push button used to increase brightness
#define BRIGHTNESS_DOWN_BUTTON_PIN   17       //Push button used to decrease broghtness
  ...
  for(int a=0; a<30; a++) {  // Repeat 30 times...
    for(int b=0; b<3; b++) { //  'b' counts from 0 to 2...
      ...
      for(int c=b; c<strip.numPixels(); c += 3) {
        ...
        uint32_t color = strip.gamma32(strip.ColorHSV(hue, 255, brightness)); // hue -> RGB
        strip.setPixelColor(c, color); // Set pixel 'c' to value 'color'
      }
      strip.show();                // Update strip with new contents
      delay(wait);                 // Pause for a moment
